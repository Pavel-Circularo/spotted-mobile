...
class SpottedSupabaseUser extends BaseAuthUser {
  SpottedSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> spottedSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SpottedSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
